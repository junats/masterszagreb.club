  ...
  const optimizeImage = async (inputBlob: Blob): Promise<Blob> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        // 1. Happy Path: Browser supports format (e.g. JPEG, PNG, or Native HEIC)
        img.onload = () => {
        ...
        // 2. Error Path: Browser cannot load format (likely HEIC on Chrome/Firefox)
        img.onerror = async () => {
          console.warn(`Browser failed to decode image. Checking if HEIC... Size: ${inputBlob.size} bytes`);

          if (inputBlob.type === 'image/heic' || inputBlob.type === 'image/heif' ||
            (inputBlob instanceof File && inputBlob.name.toLowerCase().endsWith('.heic'))) {

            try {
              console.log('🔄 Native decode failed. Starting HEIC conversion...');

              // Retry Helper
              const attemptConversion = async (q: number, t: number) => {
                const p = heic2any({ blob: inputBlob, toType: 'image/jpeg', quality: q });
                const timer = new Promise((_, r) => setTimeout(() => r(new Error(`Timeout ${t}ms`)), t));
                // @ts-ignore
              ...
              let converted;
              try {
                // Extended timeout (60s) for first attempt
                console.log("Attempting Standard Conversion...");
                converted = await attemptConversion(0.3, 60000);
              } catch (e) {
                console.warn("Retrying HEIC conversion with low quality...");
                // Desperation attempt: 60s timeout, very low quality
                converted = await attemptConversion(0.1, 60000);
              }

              const convertedBlob = Array.isArray(converted) ? converted[0] : converted as Blob;
              console.log('✅ HEIC Converted. Recursing to optimize...');

              // RECURSION
              const finalBlob = await optimizeImage(convertedBlob);
              resolve(finalBlob);

            } catch (e) {
              console.error("HEIC Conversion Failed:", e);
              console.warn("⚠️ Proceeding with original file as last resort...");
              // FAIL-OPEN: Send original file. If backend fails, so be it. 
              // This is better than blocking the user if the server IS updated.
              resolve(inputBlob);
            }
          } else {
            console.error("Unsupported Image Format:", inputBlob.type);
            reject(new Error("Image format not supported by browser."));
          }
        };
      };
      ...
      >
        ...
        {isAnalyzing && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-sm p-8 rounded-3xl border border-slate-800 shadow-2xl flex flex-col items-center justify-center text-center">
