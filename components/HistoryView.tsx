...
const HistoryView: React.FC<HistoryViewProps> = ({ receipts, ageRestricted, onDelete, onUpdate }) => {
  ...
  return (
    <div className="flex flex-col h-full px-4 pt-4 pb-24 bg-background">
      <div className="mb-6">
        <h1 className="text-2xl font-heading font-bold text-white tracking-tight">History</h1>
        <p className="text-slate-400 text-sm font-medium">Review receipts and bills {ageRestricted && <span className="text-amber-500">(Filtered)</span>}</p>
      </div>
            ...
            filteredReceipts.map((receipt) => {
                ...
                return (
                    <button 
                        ...
                        className={`w-full transition-all duration-300 p-3 rounded-2xl border flex items-center gap-4 group ${
                            isBill 
