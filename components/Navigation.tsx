...
const Navigation: React.FC<NavigationProps> = ({ currentView, setView, isVisible }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-surface/80 backdrop-blur-xl border-t border-white/5 pb-safe pt-2 px-2 h-20 z-50 transition-all duration-300">
      <div className="flex justify-between items-center max-w-md mx-auto h-full pb-2">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`p-2 rounded-full transition-all duration-500 ease-out relative ${
              ...
              <span className={`text-[10px] font-medium truncate w-full text-center transition-colors duration-300 ${
